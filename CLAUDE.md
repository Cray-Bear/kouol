...
本项目采用六权协作模型（策/议/行/审/督 + 用户 + 记分板），**没有任何一个角色可以独自决定所有事情**。

| 权 | 角色 | 核心职责 |
|----|------|---------|
| 策+产品 | 🎯 Strategist | 产生想法、定义需求、拆解任务 |
| 议 | 🏗️ Architect + 🎨 Designer | 三方讨论形成方案（战略者代替 PM 参与） |
| 行+运维 | 💻 Developer | 按方案实施 + 部署验证 |
...
**博客/网站变更必须本地验证后再 push：**

```
开发完成 → bundle exec jekyll serve（本地）→ curl 验证关键 URL 无 404 → commit & push → curl 线上验证
```
